...
class Weibo:
                ...
                if len(info) > 3:
                    for i in range(0, len(info) - 2):
                        ...
                        try:
                            ...
                            str_zan = info[i].xpath("div/a/text()")[0]
                        guid = re.findall(pattern, str_zan, re.M)
                        if guid == []:
                            str_zan = info[i].xpath("div/span[@class='cmt']/text()")[0]
                            guid = re.findall(pattern, str_zan, re.M)
                        up_num = int(guid[0])
                        ...
                        #获得多组图片
                        pic_url = info[i].xpath('div[1]/a/@href')
                        if len(pic_url)==1:
                            self.pic_url.append(pic_url[0])
                        #获取1组图片
                        pic_url = info[i].xpath('div[2]/a[1]/@herf')
                        if len(pic_url)==1:
                            self.pic_url.append(pic_url[0])
    ...
    def down_picture(self):
        # 获取微博图片的url
        self.pic_url=['https://weibo.cn/mblog/picAll/FxPSR8CPH?rl=1']
        for url in self.pic_url:
            print url
            html = requests.get(url, cookies=self.cookie).content
            selector = etree.HTML(html)
            pics_divs = selector.xpath('//div[@class="c"]')[1:-1]

            for pic_div in pics_divs:
                imageName = pic_div.xpath('a/img/@src')[0]
                content = req = requests.get(imageName).content
                with open("E:\\test_ui", 'wb') as fp:
                    fp.write(content)
...
def main():
    try:
        ...
        user_id = 2612352067  # 可以改成任意合法的用户id（爬虫的微博id除外）
